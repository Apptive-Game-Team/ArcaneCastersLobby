...
/**
 * `indicator` carries a jsonb document from `MagicQueryRepository` as a plain `String`
 * (selected with `m.indicator::text`). The lobby must not parse or validate that document,
 * only pass it through to the client inline as a JSON object, so `MagicDto.indicator` is
 * annotated `@JsonRawValue`. This confirms Jackson emits the string's content verbatim
 * instead of a quoted, escaped string, and that a null indicator still serializes as a
 * JSON `null` rather than the quoted text `"null"`.
 */
class MagicDtoSerializationTest {
    ...
    fun `null indicator serializes as JSON null, not the literal text null`() {
        val dto = MagicDto(1L, "fireball", "Fire", listOf("Fire"), 15, null)
